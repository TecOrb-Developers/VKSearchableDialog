package com.vkp.searchabledialog
...
import com.vkp.searchabledialog.adapter.DialogAdapter
import com.vkp.searchabledialog.callback.OnAnimationEnd
import com.vkp.searchabledialog.callback.OnItemSelectedListener
import com.vkp.searchabledialog.utils.CircularReveal
...
class SearchableDialog(private val context: Context) : LifecycleObserver {
    ...
    var windowTitleTextColor: Int = ContextCompat.getColor(context, android.R.color.holo_red_dark)
        @ColorInt get() = field
        set(@ColorInt colorInt) {
            field = colorInt
        }
    ...
    fun setSpinnerListItems(spinnerList: ArrayList<String>) {
        recyclerAdapter =
            DialogAdapter(context, spinnerList, object : OnItemSelectedListener {
                override fun setOnItemSelectedListener(position: Int, selectedString: String) {
                    selectedItemPosition = position
                    selectedItem = selectedString
                    if (dismissSpinnerOnItemClick) dismiss()
                    if (::onItemSelectedListener.isInitialized) onItemSelectedListener.setOnItemSelectedListener(
                        position,
                        selectedString
                    )
                }
